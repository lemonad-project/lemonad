...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='lemonad.conf'
CONFIGFOLDER='/root/.lemonad'
COIN_DAEMON='lemonadd'
COIN_CLI='lemonad-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/lemonad-project/lemonad/releases/download/v1.0.1.1/lemonad-1.0.1.1-x86_x64-linux.tar.gz'
...
function sync_node() {
  ...
  rm -r {blocks,database,fee_estimates.dat,mnpayments.dat,lemonadd.pid,budget.dat,db.log,peers.dat,chainstate,debug.log,mncache.dat} >/dev/null 2>&1
...
function download_node() {
  echo -e "Preparing to download ${GREEN}$COIN_NAME${NC} - ${COIN_ZIP} archive."
