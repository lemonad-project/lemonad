...
{   
    ...
    CAmount devFund = GetDevFee(pindexPrev->nHeight, blockValue);
    ...
    if (devFund > 0){
        unsigned int i = txNew.vout.size();
        txNew.vout.resize(i + 1);
        txNew.vout[i].scriptPubKey = GetScriptForDestination(CBitcoinAddress(Params().DevAddress()).Get());
        txNew.vout[i].nValue = devFund;
        LogPrint("devfund","Dev fund payment for %lld\n", devFund);
    }
}
